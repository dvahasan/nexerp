...
export default function UserModal({ open, onClose, editUser = null }) {
  ...
  const inputCls = "w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm transition-all";
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {ROLES.map(r => (
              <button
                ...
                className={`py-2.5 px-3 rounded-xl text-sm font-bold capitalize transition-all border-2 ${
                  form.role === r
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button type="submit" disabled={saving} className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/30 flex items-center gap-2 disabled:opacity-70">
