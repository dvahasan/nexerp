...
export default function Projects() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (editItem) await api.updateProject(editItem._id, form);
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, margin: 0, color: t.fg }}>Projects</h1>
          <p style={{ color: t.fgMuted, margin: '4px 0 0', fontSize: 14 }}>Manage internal or external projects</p>
        </div>
        <button onClick={() => handleOpen()} style={{
          ...
          <Icon name="add" size={18} /> Add Project
        </button>
      </div>
      ...
      <div style={{ backgroundColor: t.canvas, borderRadius: 4, border: `1px solid ${t.border}`, overflow: 'auto' }}>
        {loading ? <div style={{ padding: 40, textAlign: 'center' }}>Loading...</div> : items.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: t.fgMuted }}>No projects found</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: isAR ? 'right' : 'left' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${t.border}`, backgroundColor: t.sunken }}>
                ...
                <th style={{ padding: 12, color: t.fgMuted, fontSize: 12 }}>Status</th>
            ...
            <tbody>
              {items.map(d => (
                <tr key={d._id} style={{ borderBottom: `1px solid ${t.border}` }}>
                  ...
                  <td style={{ padding: 12, color: t.fgSubtle }}>{d.status}</td>
                  <td style={{ padding: 12 }}>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Project' : 'Add Project'}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: 6, fontSize: 12, color: t.fgSubtle }}>Status</label>
            <select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} style={inputStyle(theme)}>
      ...
      <Confirm open={confirmOpen} title="Delete Project" msg="Are you sure you want to delete this?" onCancel={() => setConfirmOpen(false)} onConfirm={handleDelete} loading={deleting} />
    </div>
  );
}
