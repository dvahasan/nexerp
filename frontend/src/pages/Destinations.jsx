...
export default function Destinations() {
  ...
  useEffect(() => {
    if (liveTx?.type === 'refresh_destinations') {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (editItem) await api.updateDestination(editItem._id, form);
      else await api.addDestination(form);
      setModalOpen(false);
      loadData();
    } catch (e) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.deleteDestination(deleteTarget);
      await loadData();
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, margin: 0, color: t.fg }}>Destinations</h1>
          <p style={{ color: t.fgMuted, margin: '4px 0 0', fontSize: 14 }}>Manage clients, branches, and outbound destinations</p>
        </div>
        <button onClick={() => handleOpen()} style={{
          ...
          <Icon name="add" size={18} /> Add Destination
        </button>
      </div>
      ...
      <div style={{ backgroundColor: t.canvas, borderRadius: 4, border: `1px solid ${t.border}`, overflow: 'auto' }}>
        {loading ? <div style={{ padding: 40, textAlign: 'center' }}>Loading...</div> : items.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: t.fgMuted }}>No destinations found</div>
        ) : (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Destination' : 'Add Destination'}>
      ...
      <Confirm open={confirmOpen} title="Delete Destination" msg="Are you sure you want to delete this?" onCancel={() => setConfirmOpen(false)} onConfirm={handleDelete} loading={deleting} />
    </div>
  );
}
